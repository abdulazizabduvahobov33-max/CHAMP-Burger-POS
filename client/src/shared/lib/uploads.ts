 ...
 * basename, always `.thumb.webp` regardless of the original's own extension. Applies uniformly
 * whether the URL is a locally-stored upload ("/uploads/xxx.jpg", thumbnail generated by
 * server/src/shared/utils/imageThumbnails.ts) or an R2-hosted one
 * (https://<bucket>.../champ-pos-uploads/xxx.jpg, thumbnail uploaded as a second object by
 * server/src/modules/uploads/upload.routes.ts under the same basename convention) — both sides
 * independently derive the same name from the same original, which is the whole reason no
 * database column is needed for "does this photo have a thumbnail".
 *
 * A pure string transform, not a lookup: whether the result actually loads is discovered by the
 * <img>'s own onError fallback (see ProductImage.tsx), never checked here. Apply this to the RAW
 * url from the API before resolveUploadUrl(), not after — the origin prefix has to end up on the
